...
def load_agent(checkpoint_path, config, device="cpu"):
    ...
    wm_state = {
        k.replace("_wm.", ""): v for k, v in agent_state.items() if k.startswith("_wm.")
    }
    task_behavior_state = {
        k.replace("_task_behavior.", ""): v
...
def main():
    ...
    total_params = sum(p.numel() for p in wm.parameters()) + sum(
        p.numel() for p in task_behavior.parameters()
    )
    ...
    for step in range(config.time_limit):
        with torch.no_grad():
            ...
            latent, _ = wm.dynamics.obs_step(
                state[0] if state else None, action_prev, embed, obs_torch["is_first"]
            )
